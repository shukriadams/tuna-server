...
# want to delete
docker run -it -v $(pwd)/.clone:/tmp/tuna shukriadams/node10build:0.0.2 sh -c "cd /tmp/tuna/src && sh ./build-frontend.sh --setup" &&
rm -rf .clone/src/node_modules &&

# build 2: run a second container install, this one npm intalls "production" modules, these we want
docker run -it -v $(pwd)/.clone:/tmp/tuna shukriadams/node10build:0.0.2 sh -c "cd /tmp/tuna/src && yarn --no-bin-links --ignore-engines --production" &&
